...
const dbs = require('../sites/dbs');
const create = require('../sites/create');
const decommission = require('../sites/decommission');
...
function fullView(site) {
  ...
  if (v.nodejs) {
    v.nodejs.actual = nodever.actual(site);
    v.nodejs.tracksLatest = nodever.isLatest(v.nodejs.node_version);
    v.nodejs.effective = nodever.effective(v.nodejs.node_version);
    v.nodejs.systemVersion = nodever.systemVersion();
  }
...
// Create a site: unix user + home, document root, stage-1 vhost template, php-fpm pool,
// self-signed certificate, vhost (nginx -t decides), logrotate. A failure unwinds all of it.
router.add('POST', '/api/sites', perm('write'), async (ctx) => {
  const b = await readJson(ctx.req);
  const r = await create.createSite(b, ctx);
  emit('site.create', ctx, r.site, 'Site ' + r.site.domain + ' created (' + r.site.type + ', user ' + r.user + ')',
    { data: { type: r.site.type, user: r.user, steps: r.steps.map((s) => s.name) } });
  health.collectSites().catch(() => {});     // so the list shows it now, not at the next sweep
  return r;
});
// Retired: a site is only ever deleted after its archive reached Telegram — see /decommission below.
router.add('DELETE', '/api/sites/:domain', perm('write'), (ctx) => {
  const site = siteOf(ctx);
  throw httpError(410, 'direct delete is disabled — sites are deleted only after their archive is delivered to Telegram',
    { detail: 'POST /api/sites/' + site.domain + '/decommission with {"confirm":"' + site.domain + '"} (GET the same URL for the plan).' });
});
// Delete with archive: plan → start (domain typed back) → poll. The site is removed only after
// Telegram has confirmed every part of the archive; see lib/sites/decommission.js.
router.add('GET', '/api/sites/:domain/decommission', perm('write'), (ctx) => decommission.plan(siteOf(ctx)));
router.add('POST', '/api/sites/:domain/decommission', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  if ((b.confirm || '') !== site.domain) throw httpError(400, 'type the domain to confirm');
  const actor = actorOf(ctx.req);
  return decommission.start(site, { databases: Array.isArray(b.databases) ? b.databases.filter((x) => typeof x === 'string') : [], cloudpanel: b.cloudpanel !== false, actor: actor && actor.user });
});
router.add('GET', '/api/decommission', perm('write'), () => ({ job: decommission.current() }));
router.add('GET', '/api/decommission/:id', perm('write'), (ctx) => {
  const j = decommission.get(ctx.params.id);
  if (!j) throw httpError(404, 'unknown job (the panel may have restarted)');
  return j;
});
router.add('GET', '/api/sites/suggest', perm('read'), (ctx) => {
  const domain = String(ctx.query.get('domain') || '').trim().toLowerCase().replace(/^www\./, '');
  return { domain, valid: vhost.validDomain(domain), user: domain ? create.suggestUser(domain) : '', userTaken: domain ? !!users.getent(create.suggestUser(domain)) : false,
    managed: domain ? !!store.get(domain) : false, vhostExists: domain ? require('fs').existsSync(vhost.confPath(domain)) : false,
    nextNodePort: create.freePort(3000), nextPoolPort: php.nextPoolPort(), types: create.TYPES };
});
...
router.add('PATCH', '/api/sites/:domain', perm('write'), async (ctx) => {
  ...
  if (patch.root_dir && patch.root_dir !== site.root_dir) {
    ...
    // Same treatment as creation: a docroot that is not group-writable locks out the site's SSH users.
    if (ent && !require('fs').existsSync(dir)) { require('../util').mkdirMode(dir, 0o770); try { require('fs').chownSync(dir, ent.uid, ent.gid); } catch (_) {} }
  }
...
router.add('PUT', '/api/sites/:domain/nodejs', perm('write'), async (ctx) => {
  ...
  if (b.port != null) {
    // Same three-way check creation uses: another site, a live listener, or the panel itself.
    n.port = Number(b.port) === site.nodejs.port ? site.nodejs.port : create.checkPort(b.port, 'app');
  }
  let nvmOut = null;
  if (b.node_version != null && String(b.node_version) !== site.nodejs.node_version) {
    const ver = String(b.node_version).trim();
    // 'latest' is not a version, it is a promise to follow the system node — nothing to install,
    // and the runtime flips to system so the unit and the app both use it.
    if (nodever.isLatest(ver)) {
      n.node_version = nodever.LATEST;
      n.runtime = 'system';
    } else {
...
/* ---- cloudflare-only, across every site ---- */
//
// The same switch as the Security tab's Cloudflare toggle, applied to a whole set of sites in one
// pass. It is a switch worth having in bulk (it is how every site here is meant to be reachable) and
// a dangerous one to apply blindly: a vhost that only allows Cloudflare's ranges answers 403 to
// everyone else, so turning it on for a domain that does not actually resolve to Cloudflare takes
// that site off the internet. Hence the plan below — nothing is applied until the caller has been
// told, per site, whether it is behind Cloudflare and whether the vhost can carry the setting.
const dnsp = require('dns').promises;

// Does this domain resolve to Cloudflare? Answers from public DNS, not from the panel's Cloudflare
// records, because a site can be proxied without this panel holding a token for its zone — and it is
// the resolved address, not the bookkeeping, that decides who reaches nginx.
async function resolvesToCloudflare(domain) {
  const addrs = [];
  for (const fn of ['resolve4', 'resolve6']) {
    try { addrs.push(...await dnsp[fn](domain)); } catch (_) { /* NODATA/NXDOMAIN for this family */ }
  }
  if (!addrs.length) return { addrs: [], viaCloudflare: null };   // null = could not tell, not "no"
  return { addrs, viaCloudflare: addrs.every((a) => router.isCloudflareIp(a)) };
}

// One site's worth of the plan. Kept separate from the apply step so both agree on what "ready" means.
async function cfOnlyPlanFor(site) {
  const dns = await resolvesToCloudflare(site.domain);
  return {
    domain: site.domain, user: site.user, type: site.type,
    cf_only: !!site.cf_only,
    has_settings_placeholder: vhost.hasPlaceholder(site.vhost_template, 'settings'),
    addrs: dns.addrs, via_cloudflare: dns.viaCloudflare,
  };
}

router.add('GET', '/api/sites/bulk/cloudflare-only', perm('read'), async () => {
  const sites = store.list();
  return { sites: await Promise.all(sites.map(cfOnlyPlanFor)) };
});

// Turn the setting on or off for one site, with the same rollback the single-site route has: if nginx
// rejects the result, the row goes back to what it was so the panel and /etc/nginx still agree.
async function setCfOnly(site, on, ctx, insertPlaceholder) {
  const before = { cf: site.cf_only, tpl: site.vhost_template };
  try {
    let s = store.update(site.id, { cf_only: !!on });
    if (!vhost.hasPlaceholder(s.vhost_template, 'settings')) {
      // Without the placeholder the rendered vhost simply drops the block: nginx would reload
      // happily and the site would stay open to the world. Refuse rather than report a success.
      if (!insertPlaceholder) throw httpError(409, 'vhost has no {{settings}} placeholder, so the setting would not reach nginx');
      const tpl = s.vhost_template.replace(/(location ~ \/\.well-known \{[\s\S]*?\n  \}\n)/, '$1\n  {{settings}}\n');
      if (!vhost.hasPlaceholder(tpl, 'settings')) throw httpError(409, 'could not find a place for {{settings}} — add it in the vhost editor');
      s = store.update(s.id, { vhost_template: tpl });
    }
    const r = await applyVhost(s, ctx, 'cloudflare-only');
    claimAnd(s, ctx, 'sites.security', (on ? 'Cloudflare-only enabled' : 'Cloudflare-only disabled') + ' for ' + s.domain + ' (bulk)', { data: { cf_only: !!on } });
...
// Sites are done one at a time, each with its own nginx -t and reload: a batch that stops at the
// first failure would leave the rest silently untouched, so every site gets its own result and the
// caller is told exactly which ones changed.
router.add('PUT', '/api/sites/bulk/cloudflare-only', perm('write'), async (ctx) => {
  const b = await readJson(ctx.req);
  if (!Array.isArray(b.domains) || !b.domains.length) throw httpError(400, 'domains must be a non-empty array — this route never guesses the set it acts on');
  const on = !!b.cf_only;
  const results = [];
  for (const d of b.domains) {
    const site = store.get(String(d));
    if (!site) { results.push({ domain: d, ok: false, error: 'unknown site' }); continue; }
    if (!!site.cf_only === on) { results.push({ domain: site.domain, ok: true, skipped: true, error: 'already ' + (on ? 'on' : 'off') }); continue; }
    try { const r = await setCfOnly(site, on, ctx, !!b.insert_settings_placeholder); results.push({ domain: site.domain, ok: true, changed: r.changed }); }
    catch (e) { results.push({ domain: site.domain, ok: false, error: e.message, detail: e.detail || null }); }
  }
  const failed = results.filter((r) => !r.ok);
  events.emit('sites.security.bulk', { req: ctx.req, message: 'Cloudflare-only ' + (on ? 'enabled' : 'disabled') + ' on ' + results.filter((r) => r.ok && !r.skipped).length + ' site(s)' + (failed.length ? ', ' + failed.length + ' failed' : ''),
    level: failed.length ? 'warn' : 'info', data: { cf_only: on, results } });
  return { ok: !failed.length, cf_only: on, results, sites: await Promise.all(store.list().map(cfOnlyPlanFor)) };
});
...
/* ---- logs ---- */
/* --------------------------------------------------------------- rules */
// Aliases, redirects, rewrites, hotlink protection and traffic limits all render into the vhost's
// {{settings}} block, so every change goes through applyVhost() — nginx -t decides, and a bad rule
// restores the previous file instead of taking the site down.
const REWRITE_FLAGS = ['last', 'break', 'redirect', 'permanent'];
const REDIRECT_KINDS = ['exact', 'prefix', 'regex'];
function rulesView(site) {
  return {
    aliases: store.aliases.list(site.id),
    redirects: store.redirects.list(site.id),
    rewrites: store.rewrites.list(site.id),
    hotlink: store.hotlink.get(site.id) || { enabled: false, extensions: 'jpg,jpeg,png,gif,webp,svg,bmp,ico,mp4,webm,mp3,pdf', allowed: null, action: 403 },
    traffic: store.traffic.get(site.id) || { enabled: false, req_per_sec: null, req_burst: 20, conn_limit: null, rate_kb: null },
    has_settings_placeholder: vhost.hasPlaceholder(site.vhost_template, 'settings'),
    zones_file: vhost.ZONES_FILE,
  };
}
// A rule nginx refuses has to leave NOTHING behind. writeWithRollback restores the file, but the row
// was already written — and a row whose rendering nginx rejects makes the site unmanageable: every
// later change re-renders the same broken vhost and fails too. So the caller passes `undo`, the row
// is removed when nginx says no, and the traffic zones (a separate include, outside the rollback)
// are rebuilt from whatever the database holds afterwards.
async function applyRules(site, ctx, type, message, opts) {
  opts = opts || {};
  vhost.writeTrafficZones();
  try {
    await applyVhost(site, ctx, type);
  } catch (e) {
    try { if (opts.undo) opts.undo(); } catch (_) {}
    vhost.writeTrafficZones();
    try { await applyVhost(site, ctx, type); } catch (_) { /* the file was already restored */ }
    throw Object.assign(e, { detail: [e.detail, 'The rule was discarded — nothing was kept, so the site is unchanged and still manageable.'].filter(Boolean).join('\n\n') });
  }
  claimAnd(site, ctx, type, message, opts.extra);
  return Object.assign({ ok: true }, rulesView(store.getById(site.id)));
}

router.add('GET', '/api/sites/:domain/rules', perm('read'), (ctx) => rulesView(siteOf(ctx)));

router.add('POST', '/api/sites/:domain/aliases', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const domain = String(b.domain || '').trim().toLowerCase();
  if (!vhost.validDomain(domain)) throw httpError(400, '"' + domain + '" is not a valid domain name');
  if (domain === site.domain) throw httpError(400, 'that is the site\'s own domain');
  if (store.get(domain)) throw httpError(409, domain + ' is a site of its own in this panel — an alias cannot shadow it');
  if (store.aliases.list(site.id).some((a) => a.domain === domain)) throw httpError(409, 'already an alias of this site');
  const aid = store.aliases.insert({ site_id: site.id, domain, redirect: !!b.redirect });
  return applyRules(site, ctx, 'sites.alias.add', 'Domain ' + domain + ' added to ' + site.domain + (b.redirect ? ' (redirects to the primary)' : ' (served)'),
    { undo: () => store.aliases.remove(aid, site.id) });
});
router.add('DELETE', '/api/sites/:domain/aliases/:id', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  if (!store.aliases.remove(Number(ctx.params.id), site.id)) throw httpError(404, 'unknown alias for this site');
  return applyRules(site, ctx, 'sites.alias.remove', 'Domain alias removed from ' + site.domain);
});

router.add('POST', '/api/sites/:domain/redirects', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const kind = REDIRECT_KINDS.includes(b.kind) ? b.kind : 'exact';
  const source = String(b.source || '').trim();
  const target = String(b.target || '').trim();
  if (!source) throw httpError(400, 'a source path is required');
  if (kind !== 'regex' && !source.startsWith('/')) throw httpError(400, 'an exact or prefix source must start with /');
  if (!/^(https?:\/\/|\/)/.test(target)) throw httpError(400, 'the target must be a full URL or an absolute path');
  if (/[;{}\n\r]/.test(source) || /[;{}\n\r]/.test(target)) throw httpError(400, 'a redirect cannot contain ; { } or a newline');
  // Two redirects on the same path become two nginx location blocks with the same name, which is a
  // hard `duplicate location` error — and once written it blocks every later change to the site.
  if (kind !== 'regex' && store.redirects.list(site.id).some((r) => r.kind !== 'regex' && r.source === source)) {
    throw httpError(409, 'there is already a redirect for ' + source + ' on this site', { detail: 'nginx would refuse the vhost with `duplicate location "' + source + '"`. Remove the existing rule first.' });
  }
  const rid = store.redirects.insert({ site_id: site.id, kind, source, target, code: Number(b.code) || 301, position: Number(b.position) || 0 });
  return applyRules(site, ctx, 'sites.redirect.add', 'Redirect added on ' + site.domain + ': ' + kind + ' ' + source + ' → ' + target,
    { undo: () => store.redirects.remove(rid, site.id) });
});
router.add('DELETE', '/api/sites/:domain/redirects/:id', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  if (!store.redirects.remove(Number(ctx.params.id), site.id)) throw httpError(404, 'unknown redirect for this site');
  return applyRules(site, ctx, 'sites.redirect.remove', 'Redirect removed from ' + site.domain);
});

router.add('POST', '/api/sites/:domain/rewrites', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const pattern = String(b.pattern || '').trim();
  const replacement = String(b.replacement || '').trim();
  if (!pattern || !replacement) throw httpError(400, 'a pattern and a replacement are both required');
  if (/[;{}\n\r]/.test(pattern) || /[;{}\n\r]/.test(replacement)) throw httpError(400, 'a rewrite cannot contain ; { } or a newline — those would end the directive');
  if (!REWRITE_FLAGS.includes(b.flag || 'last')) throw httpError(400, 'flag must be one of ' + REWRITE_FLAGS.join(', '));
  if (store.rewrites.list(site.id).some((r) => r.pattern === pattern && r.replacement === replacement)) throw httpError(409, 'that exact rewrite is already there');
  const wid = store.rewrites.insert({ site_id: site.id, pattern, replacement, flag: b.flag || 'last', position: Number(b.position) || 0 });
  return applyRules(site, ctx, 'sites.rewrite.add', 'Rewrite added on ' + site.domain + ': ' + pattern + ' → ' + replacement,
    { undo: () => store.rewrites.remove(wid, site.id) });
});
router.add('DELETE', '/api/sites/:domain/rewrites/:id', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  if (!store.rewrites.remove(Number(ctx.params.id), site.id)) throw httpError(404, 'unknown rewrite for this site');
  return applyRules(site, ctx, 'sites.rewrite.remove', 'Rewrite removed from ' + site.domain);
});

router.add('PUT', '/api/sites/:domain/hotlink', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const patch = {};
  if (b.enabled !== undefined) patch.enabled = !!b.enabled;
  if (b.extensions !== undefined) {
    const exts = String(b.extensions).split(',').map((x) => x.trim().toLowerCase()).filter(Boolean);
    if (exts.some((x) => !/^[a-z0-9]{1,8}$/.test(x))) throw httpError(400, 'extensions must be plain letters/digits, comma separated (jpg,png,mp4)');
    patch.extensions = exts.join(',');
  }
  if (b.allowed !== undefined) {
    const hosts = String(b.allowed || '').split(/[\s,]+/).map((x) => x.trim()).filter(Boolean);
    if (hosts.some((h) => !/^[A-Za-z0-9.*_-]+$/.test(h))) throw httpError(400, 'allowed referers must be host names, optionally with * (cdn.example.com, *.partner.net)');
    patch.allowed = hosts.join('\n') || null;
  }
  if (b.action !== undefined) { const a = Number(b.action); if (![403, 444].includes(a)) throw httpError(400, 'action must be 403 (forbidden) or 444 (drop the connection)'); patch.action = a; }
  const prevH = store.hotlink.get(site.id);
  store.hotlink.set(site.id, patch);
  const h = store.hotlink.get(site.id);
  return applyRules(site, ctx, 'sites.hotlink', 'Hotlink protection ' + (h.enabled ? 'enabled (' + h.extensions + ')' : 'disabled') + ' on ' + site.domain,
    { undo: () => { if (prevH) store.hotlink.set(site.id, prevH); else store.hotlink.set(site.id, { enabled: false }); } });
});

router.add('PUT', '/api/sites/:domain/traffic', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const patch = {};
  const num = (v, min, max, what) => { if (v === null || v === '' || v === undefined) return null; const n = Number(v); if (!Number.isFinite(n) || n < min || n > max) throw httpError(400, what + ' must be between ' + min + ' and ' + max); return Math.round(n); };
  if (b.enabled !== undefined) patch.enabled = !!b.enabled;
  if (b.req_per_sec !== undefined) patch.req_per_sec = num(b.req_per_sec, 1, 10000, 'requests per second');
  if (b.req_burst !== undefined) patch.req_burst = num(b.req_burst, 1, 10000, 'burst') || 20;
  if (b.conn_limit !== undefined) patch.conn_limit = num(b.conn_limit, 1, 10000, 'concurrent connections');
  if (b.rate_kb !== undefined) patch.rate_kb = num(b.rate_kb, 8, 1000000, 'bandwidth in KB/s');
  const prevT = store.traffic.get(site.id);
  store.traffic.set(site.id, patch);
  const t = store.traffic.get(site.id);
  if (t.enabled && !t.req_per_sec && !t.conn_limit && !t.rate_kb) {
    if (prevT) store.traffic.set(site.id, prevT); else store.traffic.set(site.id, { enabled: false });
    throw httpError(400, 'traffic control is on but no limit is set — set requests/second, connections or bandwidth');
  }
  return applyRules(site, ctx, 'sites.traffic', 'Traffic control ' + (t.enabled ? 'on (' + [t.req_per_sec ? t.req_per_sec + ' r/s' : null, t.conn_limit ? t.conn_limit + ' conn' : null, t.rate_kb ? t.rate_kb + ' KB/s' : null].filter(Boolean).join(', ') + ')' : 'off') + ' for ' + site.domain,
    { undo: () => { if (prevT) store.traffic.set(site.id, prevT); else store.traffic.set(site.id, { enabled: false }); } });
});

/* --------------------------------------------------------------- databases */
const MAX_DUMP = 4 * 1024 * 1024 * 1024;   // 4 GB streamed straight into the client, never buffered
router.add('GET', '/api/sites/:domain/databases', perm('read'), (ctx) => dbs.list(siteOf(ctx)));
router.add('POST', '/api/sites/:domain/databases', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const r = await dbs.create(site, b);
  emit('site.db.create', ctx, site, 'Database ' + r.name + ' (' + r.engine + ') created for ' + site.domain + ' with user ' + r.user,
    { data: { engine: r.engine, name: r.name, user: r.user, permissions: r.permissions } });
  return r;
});
router.add('DELETE', '/api/sites/:domain/databases/:id', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const r = await dbs.drop(site, Number(ctx.params.id), { keepData: ctx.query.get('keepData') === '1', dropUsers: ctx.query.get('keepUsers') !== '1' });
  emit('site.db.drop', ctx, site, 'Database ' + r.name + ' (' + r.engine + ') ' + (r.dropped ? 'dropped' : 'unlinked, data kept'), { level: 'warn' });
  return r;
});
router.add('POST', '/api/sites/:domain/databases/adopt', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const r = dbs.adopt(site, b.engine, b.name);
  emit('site.db.adopt', ctx, site, 'Database ' + r.name + ' (' + r.engine + ') taken under management');
  return r;
});
router.add('GET', '/api/sites/:domain/databases/:id/tables', perm('read'), (ctx) => dbs.tables(siteOf(ctx), Number(ctx.params.id)));
router.add('POST', '/api/sites/:domain/databases/:id/export', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const r = await dbs.exportDb(site, Number(ctx.params.id));
  emit('site.db.export', ctx, site, 'Database ' + r.name + ' exported (' + Math.round(r.size / 1024) + ' KB) to ' + r.file);
  return r;
});
// The dump streams from the request into the client's stdin — nothing is buffered or written to disk.
router.add('POST', '/api/sites/:domain/databases/:id/import', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const r = await dbs.importDb(site, Number(ctx.params.id), ctx.req, MAX_DUMP);
  emit('site.db.import', ctx, site, 'Dump loaded into ' + r.name + ' (' + Math.round(r.bytes / 1024) + ' KB read)', { level: 'warn', data: { bytes: r.bytes } });
  return r;
});
router.add('POST', '/api/sites/:domain/databases/:id/maintain/:action', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const r = await dbs.maintain(site, Number(ctx.params.id), ctx.params.action);
  emit('site.db.maintain', ctx, site, ctx.params.action + ' run on ' + site.domain + "'s database");
  return r;
});
router.add('PUT', '/api/sites/:domain/databases/:id/users/:uid', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const b = await readJson(ctx.req);
  const out = {};
  if (b.permissions !== undefined) Object.assign(out, await dbs.setUserPermissions(site, Number(ctx.params.id), Number(ctx.params.uid), b.permissions));
  if (b.password !== undefined || b.regenerate) Object.assign(out, await dbs.setUserPassword(site, Number(ctx.params.id), Number(ctx.params.uid), b.password));
  if (!Object.keys(out).length) throw httpError(400, 'nothing to change (send permissions and/or password)');
  emit('site.db.user', ctx, site, 'Database user updated' + (b.permissions ? ' → ' + b.permissions : '') + (b.password !== undefined || b.regenerate ? ' (new password)' : ''));
  return out;
});
router.add('GET', '/api/sites/:domain/databases/:id/users/:uid/password', perm('write'), (ctx) => dbs.revealUserPassword(siteOf(ctx), Number(ctx.params.id), Number(ctx.params.uid)));

/* ------------------------------------------------- database servers (root password manager) */
router.add('GET', '/api/db-servers', { perm: 'sites.read' }, async () => ({ servers: await Promise.all(dbs.ENGINES.map((e) => dbs.testAdmin(e))) }));
router.add('PUT', '/api/db-servers/:engine', { perm: 'sites.write' }, async (ctx) => {
  const b = await readJson(ctx.req);
  const r = await dbs.setAdmin(ctx.params.engine, b.password, b.admin_user);
  events.emit('db.admin.set', { req: ctx.req, target: ctx.params.engine, message: 'Admin credential stored for ' + ctx.params.engine + ' (' + r.admin_user + ')' });
  return r;
});
router.add('POST', '/api/db-servers/mariadb/import-cloudpanel', { perm: 'sites.write' }, async (ctx) => {
  const r = await dbs.importAdminFromCloudPanel();
  events.emit('db.admin.set', { req: ctx.req, target: 'mariadb', message: 'MariaDB root password imported from CloudPanel and verified' });
  return r;
});
router.add('GET', '/api/db-servers/:engine/password', { perm: 'sites.write' }, (ctx) => dbs.revealAdmin(ctx.params.engine));
// Last resort when nobody holds MariaDB's root password: --init-file via systemd (never
// --skip-grant-tables, which would drop authentication server-wide for the duration).
router.add('POST', '/api/db-servers/mariadb/reset-root', { perm: 'sites.write' }, async (ctx) => {
  const r = await dbs.resetMariadbRoot();
  events.emit('db.admin.reset', { req: ctx.req, target: 'mariadb', level: 'warn',
    message: "MariaDB root authentication reset: unix_socket for local root plus a new generated password, stored encrypted",
    data: { steps: r.steps.map((x) => x.name), version: r.version } });
  return r;
});
...
/* --------------------------------------------------------------- processes */
router.add('GET', '/api/sites/:domain/processes', perm('read'), (ctx) => procs.list(siteOf(ctx), { live: true }));
...
router.add('POST', '/api/sites/:domain/processes/:action', perm('write'), async (ctx) => {
  ...
  // pm2 returns as soon as it has signalled the process; the app itself needs a moment to bind its
  // port (or to let go of it). Re-reading health before that settles reports the state we just left.
  await new Promise((res) => setTimeout(res, 1500));
  let fresh = null;
  try { fresh = await health.refreshSite(site.domain); } catch (_) {}
  // A stop that a cron will undo is worth saying out loud, once, at the moment it is made.
  let watchdogs = [];
  if (ctx.params.action === 'stop') { try { watchdogs = procs.watchdogCrons(site); } catch (_) {} }
  emit('site.pm2.' + ctx.params.action, ctx, site, 'pm2 ' + ctx.params.action + ' ' + (app || 'all') + ' · ' + site.domain + (failed.length ? ' failed' : ''),
    { level: failed.length ? 'error' : 'info', data: { results: r.results.map((x) => ({ user: x.user, ok: x.ok, command: x.command, output: (x.output || '').slice(0, 500) })),
      status: fresh ? fresh.status : null, statusReason: fresh ? fresh.statusReason : null, watchdogs } });
  return Object.assign({}, r, { health: fresh, watchdogs });
});
