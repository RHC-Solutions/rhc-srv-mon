...
// Requires the domain typed back, so a mis-click cannot delete a site.
router.add('DELETE', '/api/sites/:domain', perm('write'), async (ctx) => {
  const site = siteOf(ctx);
  const confirm = ctx.query.get('confirm') || '';
  if (confirm !== site.domain) throw httpError(400, 'type the domain to confirm', { detail: 'Pass ?confirm=' + site.domain + ' — this removes the vhost, certificates, php pool, databases, SSH/FTP users and the unix account.' });
  const r = await create.deleteSite(site, { keepHome: ctx.query.get('keepHome') === '1', keepUser: ctx.query.get('keepUser') === '1', dropDatabases: ctx.query.get('keepDatabases') !== '1' });
  events.emit('site.delete', { req: ctx.req, site: r.domain, level: 'warn', message: 'Site ' + r.domain + ' deleted' + (r.errors.length ? ' with ' + r.errors.length + ' problem(s)' : ''), data: { steps: r.steps.map((s) => s.name), errors: r.errors } });
  health.collectSites().catch(() => {});
  return r;
});
