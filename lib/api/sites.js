...
async function applyVhost(site, ctx, what) {
  ...
  if (!r.ok) {
    ...
    throw httpError(409, 'nginx -t failed — previous vhost restored', { detail: (r.test && r.test.output) || (r.reload && r.reload.output) });
  }
  return r;
}
...
function fullView(site) {
  ...
  v.health = h ? { status: h.status, statusLabel: h.statusLabel, httpUp: h.httpUp, portUp: h.portUp, disk: h.disk, pm2: h.pm2 } : null;
  return v;
}
...
router.add('GET', '/api/sites/runtimes', perm('read'), () => ({
  php: php.listVersions(), nextPoolPort: php.nextPoolPort(), usedPorts: [...store.usedPorts()].sort((a, b) => a - b),
  node: { system: process.version, nvmDefault: '22' }, ip: publicIp(),
}));
...
router.add('PUT', '/api/sites/:domain/php', perm('write'), async (ctx) => {
  ...
  const r = await applyVhost(store.getById(site.id), ctx, 'php settings').catch((e) => { store.setPhp(site.id, site.php); throw e; });
...
router.add('PUT', '/api/sites/:domain/nodejs', perm('write'), async (ctx) => {
  ...
  const r = await applyVhost(next, ctx, 'node settings').catch((e) => { store.setNodejs(site.id, site.nodejs); throw e; });
