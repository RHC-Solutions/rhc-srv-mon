...
router.add('POST', '/api/settings/telegram/test', { perm: 'settings.write' }, async (ctx) => {
  const t = notify.getConfig();
  if (!t || !t.botToken || !t.chatId) throw httpError(400, 'save a bot token and chat ID first');
...
router.add('POST', '/api/settings/slack/test', { perm: 'settings.write' }, async (ctx) => {
  const s = notify.getSlack();
  if (!s || !s.webhookUrl) throw httpError(400, 'save a webhook URL first');
...
router.add('PUT', '/api/settings/cloudflare', { perm: 'settings.write' }, async (ctx) => {
  ...
  if (!v || v.status !== 'active') throw httpError(400, 'token is ' + ((v && v.status) || 'invalid'));
  cloudflare.setToken(t);
  events.emit('settings.save', { req: ctx.req, target: 'cloudflare', message: 'Cloudflare API token saved (verified, id ' + v.id.slice(0, 8) + '…)' });
  return { ok: true, configured: true, token: cloudflare.masked(), verify: v };
});
router.add('POST', '/api/settings/cloudflare/test', { perm: 'settings.write' }, async () => {
  const v = await cloudflare.verify();
  const z = await cloudflare.zones();
  return { ok: true, status: v.status, expires_on: v.expires_on || null, zones: z.length };
});
