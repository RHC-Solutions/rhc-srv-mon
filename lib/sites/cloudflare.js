...
async function status(site, serverIp) {
  ...
  const out = {
    ...
    points_here: null, proxied: null, serverIp: serverIp || null, error: null,
  ...
  try {
    ...
    out.error = e.message;
    // A token that lists zones but cannot read their records is missing one specific permission, and
    // Cloudflare says only "Authentication error" — name the row to tick.
    out.errorDetail = e.detail || (e.cfCode === 10000 || /10000/.test(e.message)
      ? 'The token can see the zone but not its DNS records, which means it is missing the DNS permission.\n\n'
        + 'In the Cloudflare dashboard, edit this token and tick the row literally called "DNS" (Grants read access to DNS) → Edit.\n'
        + 'Note that "Zone DNS Settings" is a different permission and does not grant access to records.'
      : null);
