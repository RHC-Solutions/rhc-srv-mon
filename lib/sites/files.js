...
const HELPER = path.join(APP_ROOT, 'bin', 'fileop.js');
...
function spawnHelper(site, argv, stdio) {
  const c = ctxFor(site);
  return spawn(process.execPath, [HELPER, '--root', c.root].concat(argv), { uid: c.uid, gid: c.gid, env: c.env, stdio: stdio || ['pipe', 'pipe', 'pipe'] });
}
...
// Stream a file to an HTTP response (as the site user).
function download(site, filePath, res, filename) {
  const child = spawnHelper(site, ['download', filePath]);
  let started = false, err = '';
  child.stderr.on('data', (d) => { err += d; });
  child.stdout.once('data', () => { started = true; });
  child.stdout.on('data', (d) => {
    if (!res.headersSent) res.writeHead(200, { 'Content-Type': 'application/octet-stream', 'Content-Disposition': 'attachment; filename="' + String(filename || path.basename(filePath)).replace(/[^\x20-\x7e]|"/g, '_') + '"', 'Cache-Control': 'no-store' });
    res.write(d);
  });
  child.on('close', (code) => {
    if (code !== 0 && !res.headersSent) { res.writeHead(/permission denied/i.test(err) ? 403 : 404, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: parseErr(err, code) })); }
    if (!res.headersSent) res.writeHead(200, { 'Content-Type': 'application/octet-stream', 'Content-Disposition': 'attachment; filename="' + path.basename(filePath) + '"' });
    res.end();
  });
  child.on('error', (e) => { if (!res.headersSent) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: e.message })); } });
}
