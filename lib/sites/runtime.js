...
const { mkdirMode, writeFileMode } = require('../util');
...
// With the system interpreter the system pm2 is already on PATH; only an nvm install needs its own.
async function installPm2(user, useSystem) {
  if (useSystem) {
    const r = await asUser(user, 'pm2 -v', { timeout: 60_000 });
    return r.ok ? { ok: true, skipped: true, output: 'system pm2 ' + r.output } : r;
  }
...
async function pm2Startup(user, version) {
  // A latest-tracking site points its unit at the system pm2 on the system PATH, so an apt upgrade
  // of node changes what the unit executes without the unit itself being rewritten.
  let bin, pm2;
  if (nodever.isLatest(version)) {
    bin = path.dirname(nodever.systemPath() || '/usr/bin/node');
    pm2 = '/usr/bin/pm2';
    if (!fs.existsSync(pm2)) return { ok: false, output: 'the system pm2 is not installed' };
  } else {
    bin = nvmBinDir(user, version);
    if (!bin) return { ok: false, output: 'no nvm interpreter to point the unit at' };
    pm2 = path.join(bin, 'pm2');
...
function seedFiles(site, port) {
  ...
  // Group-writable: the app may run as one of the site's SSH users, and pm2 writes these logs.
  try { mkdirMode(logs, 0o770); fs.chownSync(logs, uid, gid); } catch (_) {}
  for (const [name, body] of [['app.js', appJs(site, port)], ['ecosystem.config.js', ecosystem(site, port, docroot)]]) {
    ...
    try { writeFileMode(f, body, 0o660); fs.chownSync(f, uid, gid); wrote.push(name); } catch (_) {}
  }
...
// Bring a site user's nvm up to the system Node and drop what it superseded.
//
// Only ever called for a site that tracks latest: there the nvm copy is a spare, and a spare that
// says v22 while everything runs v26 is exactly the stale record this host was full of. A site that
// deliberately pins a version is left alone — keeping that version *is* the point of pinning.
// A user with no nvm, or an nvm holding no Node at all, is left as it is: there is nothing to bring
// up to date, and installing one would be a new install rather than an update.
async function syncNvmToSystem(user) {
  if (!USER_RE.test(user)) throw Object.assign(new Error('invalid unix user'), { status: 400 });
  const dir = nvmDir(user);
  if (!fs.existsSync(nvmSh(user))) return { ok: true, skipped: true, output: 'no nvm' };
  let have = [];
  try { have = fs.readdirSync(path.join(dir, 'versions', 'node')).filter((n) => /^v\d+\./.test(n)); } catch (_) { have = []; }
  if (!have.length) return { ok: true, skipped: true, output: 'nvm present but empty' };

  const target = nodever.systemVersion();
  if (!target) return { ok: false, output: 'no system node to match' };
  const bare = target.replace(/^v/, '');
  if (have.length === 1 && have[0] === target) return { ok: true, skipped: true, output: 'already ' + target };

  const inst = await asUser(user, withNvm('nvm install ' + bare + ' && nvm alias default ' + bare), { timeout: 10 * 60_000 });
  if (!inst.ok) return inst;
  const removed = [];
  for (const v of have) {
    if (v === target) continue;
    const r = await asUser(user, withNvm('nvm uninstall ' + v.replace(/^v/, '')), { timeout: 5 * 60_000 });
    if (r.ok) removed.push(v);
  }
  return { ok: true, output: 'now ' + target + (removed.length ? ', removed ' + removed.join(', ') : '') };
}

// The counterpart to the pm2/systemd half of provisioning. `userdel -r` takes the home and the
// processes, but a unit in /etc/systemd/system and an entry in /var/lib/systemd/linger are outside
// it — left behind they are an enabled service pointing at a home that no longer exists, which
// fails on every boot and accumulates one orphan per deleted site.
async function deprovisionNode(user) {
  if (!USER_RE.test(user)) throw Object.assign(new Error('invalid unix user'), { status: 400 });
  const unit = 'pm2-' + user + '.service';
  const out = [];
  if (fs.existsSync('/etc/systemd/system/' + unit)) {
    const d = await run(['systemctl', 'disable', '--now', unit], { timeout: 120_000 });
    out.push('disabled ' + unit + (d.ok ? '' : ' (' + d.output + ')'));
    try { fs.unlinkSync('/etc/systemd/system/' + unit); out.push('removed ' + unit); } catch (e) { out.push('could not remove ' + unit + ': ' + e.message); }
    await run(['systemctl', 'daemon-reload'], { timeout: 60_000 });
    await run(['systemctl', 'reset-failed', unit], { timeout: 30_000 }).catch(() => null);
  }
  const lingerFile = '/var/lib/systemd/linger/' + user;
  if (fs.existsSync(lingerFile)) {
    const l = await run(['loginctl', 'disable-linger', user], { timeout: 30_000 });
    // loginctl resolves the account name first, so it is no help once the user is already gone —
    // and this also runs as a repair on sites deleted before the unit was cleaned up at all. The
    // file it manages is then the only thing left to remove.
    if (l.ok && !fs.existsSync(lingerFile)) out.push('disabled linger');
    else { try { fs.unlinkSync(lingerFile); out.push('removed the linger entry directly'); } catch (e) { out.push('could not remove linger: ' + e.message); } }
  }
  return { ok: true, output: out.length ? out.join('; ') : 'nothing to remove' };
}
...
async function provisionNode(site, port, say) {
  ...
  // A site that tracks latest runs the system interpreter, so there is nothing to install: an nvm
  // copy would be a second Node frozen at today's version, which is the pinning this is meant to
  // avoid. Only a site that asked for a specific version gets its own nvm.
  const tracksLatest = nodever.isLatest(site.nodejs.node_version);
  let runtimeKind = 'system';
  if (tracksLatest) {
    const v = nodever.systemVersion();
    if (!v) return { ok: false, steps, runtime: null };
    steps.push({ name: 'using the system Node.js ' + v, ok: true, output: nodever.systemPath() });
    say('runtime: using the system Node.js ' + v, nodever.systemPath() + ' — tracks the system node, so an upgrade applies here on the next restart');
  } else {
    runtimeKind = 'nvm';
    const nvm = await step('installed nvm ' + NVM_VERSION, () => installNvm(site.user));
    if (!nvm.ok) return { ok: false, steps, runtime: null };
    ...
    if (!node.ok) return { ok: false, steps, runtime: null };
  }
  const pm2 = await step('installed pm2', () => installPm2(site.user, tracksLatest));
  if (!pm2.ok) return { ok: false, steps, runtime: null };
  ...
  return { ok: steps.every((s) => s.ok), steps, runtime: runtimeKind };
}

module.exports = { provisionNode, deprovisionNode, syncNvmToSystem, installNvm, installNode, installPm2, enableLinger, pm2Startup, wireProfile, seedFiles, startApp, nvmBinDir, NVM_VERSION };
