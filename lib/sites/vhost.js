...
// Aliases are extra server_names. On an imported site the stored template has a literal server_name,
// so patchServerName() rewrites that line instead; a template still holding {{server_name}} gets them
// from here.
function aliasNames(site) {
  try { return store.aliases.list(site.id).map((a) => a.domain); } catch (_) { return []; }
}
...
/* ---- per-site rules that render into {{settings}} ---- */
// Rate-limit zones have to be declared in nginx's http context, and this box's nginx.conf includes
// only sites-enabled/*.conf — which IS spliced into http. A 000-prefixed file there therefore holds
// the zones without touching nginx.conf (which CloudPanel owns), and sorts before every vhost.
const ZONES_FILE = path.join(SITES_DIR, '000-rhc-traffic-zones.conf');
function writeTrafficZones() {
  const rows = store.traffic.all();
  const lines = ['# Generated by rhc-srv-mon. Rate-limit zones for sites with traffic control on.',
    '# Not a vhost: these are http-context directives, and sites-enabled/*.conf is included in http.'];
  for (const t of rows) {
    if (t.req_per_sec) lines.push('limit_req_zone $binary_remote_addr zone=rhc_req_' + t.site_id + ':10m rate=' + Number(t.req_per_sec) + 'r/s;');
    if (t.conn_limit) lines.push('limit_conn_zone $binary_remote_addr zone=rhc_conn_' + t.site_id + ':10m;');
  }
  const body = lines.join('\n') + '\n';
  let cur = null; try { cur = fs.readFileSync(ZONES_FILE, 'utf8'); } catch (_) {}
  if (cur === body) return { changed: false, file: ZONES_FILE };
  if (lines.length === 2) { try { fs.unlinkSync(ZONES_FILE); return { changed: true, file: ZONES_FILE, removed: true }; } catch (_) { return { changed: false, file: ZONES_FILE }; } }
  fs.writeFileSync(ZONES_FILE, body, { mode: 0o644 });
  return { changed: true, file: ZONES_FILE };
}
function trafficBlock(site) {
  const t = store.traffic.get(site.id);
  if (!t || !t.enabled) return '';
  const out = [];
  if (t.req_per_sec) out.push('  limit_req zone=rhc_req_' + site.id + ' burst=' + (Number(t.req_burst) || 20) + ' nodelay;');
  if (t.conn_limit) out.push('  limit_conn rhc_conn_' + site.id + ' ' + Number(t.conn_limit) + ';');
  if (t.rate_kb) out.push('  limit_rate ' + Number(t.rate_kb) + 'k;');
  return out.join('\n');
}
// Hotlink protection without shadowing any location. A `location ~*` for media extensions would take
// precedence over the site's own handlers and break anything the app serves itself, so the check is
// two `set`s combined in the server block — the standard way to and-together conditions in nginx.
const HOTLINK_EXT_RE = /^[A-Za-z0-9]{1,8}$/;
function hotlinkBlock(site) {
  const h = store.hotlink.get(site.id);
  if (!h || !h.enabled) return '';
  const exts = String(h.extensions || '').split(',').map((x) => x.trim().toLowerCase()).filter((x) => HOTLINK_EXT_RE.test(x));
  if (!exts.length) return '';
  const extra = String(h.allowed || '').split(/[\s,]+/).map((x) => x.trim()).filter((x) => /^[A-Za-z0-9.*_-]+$/.test(x));
  const action = Number(h.action) === 444 ? 'return 444;' : 'return 403;';
  return ['  valid_referers none blocked server_names' + (extra.length ? ' ' + extra.join(' ') : '') + ';',
    '  set $rhc_hotlink "";',
    '  if ($invalid_referer) { set $rhc_hotlink "R"; }',
    '  if ($uri ~* "\\.(' + exts.join('|') + ')$") { set $rhc_hotlink "${rhc_hotlink}U"; }',
    '  if ($rhc_hotlink = "RU") { ' + action + ' }'].join('\n');
}
const REDIRECT_CODES = new Set([301, 302, 303, 307, 308]);
function redirectBlock(site) {
  const rows = store.redirects.list(site.id);
  if (!rows.length) return '';
  const out = [];
  for (const r of rows) {
    const code = REDIRECT_CODES.has(Number(r.code)) ? Number(r.code) : 301;
    const target = String(r.target);
    if (r.kind === 'regex') out.push('  rewrite ' + r.source + ' ' + target + ' ' + (code === 302 ? 'redirect' : 'permanent') + ';');
    else if (r.kind === 'prefix') out.push('  location ^~ ' + r.source + ' {\n    return ' + code + ' ' + target + ';\n  }');
    else out.push('  location = ' + r.source + ' {\n    return ' + code + ' ' + target + ';\n  }');
  }
  return out.join('\n');
}
const REWRITE_FLAGS = new Set(['last', 'break', 'redirect', 'permanent']);
function rewriteBlock(site) {
  const rows = store.rewrites.list(site.id);
  if (!rows.length) return '';
  return rows.map((r) => '  rewrite ' + r.pattern + ' ' + r.replacement + ' ' + (REWRITE_FLAGS.has(r.flag) ? r.flag : 'last') + ';').join('\n');
}
// An alias marked `redirect` is sent to the primary domain instead of being served from it.
function aliasRedirectBlock(site) {
  const rows = store.aliases.list(site.id).filter((a) => a.redirect);
  if (!rows.length) return '';
  return ['  if ($host ~* ^(' + rows.map((a) => a.domain.replace(/\./g, '\\.')).join('|') + ')$) {',
...
function settingsBlock(site) {
  ...
  // Order is deliberate: limits and the hotlink guard apply to everything below them; alias
  // redirects and rewrites must run before any location is chosen.
  for (const fn of [trafficBlock, hotlinkBlock, aliasRedirectBlock, rewriteBlock, redirectBlock]) {
    const b = fn(site);
    if (b) el.push(b);
  }
  return el.join('\n\n');
...
function render(site, stored) {
  ...
  let out = stripPlaceholders(fill(tpl, map));
  out = addAliasNames(out, site);
  return out.endsWith('\n') ? out : out + '\n';
}
// Extra server_names for aliases that are served (not redirected). Patched at render time rather
// than baked into the stored template, so it works whether the template still holds
// {{server_name}} or an imported literal — and with no aliases it is a no-op, which is what keeps
// render() byte-identical to the file on disk for every imported site.
function addAliasNames(content, site) {
  // EVERY alias goes into server_name, redirecting ones included: nginx has to select this block
  // before the `if ($host …)` redirect can run at all. Left out, a redirect alias reaches the
  // default server instead, which has no certificate for it — the TLS handshake fails and the
  // browser shows a connection error rather than a redirect.
  const extra = aliasNames(site);
  if (!extra.length) return content;
  const own = String(site.domain).toLowerCase();
  const re = /^([ \t]*)server_name\s+([^;]+);/gm;
  return content.replace(re, (line, indent, names) => {
    const list = names.trim().split(/\s+/);
    // only the block that serves this site, never the www→apex redirect block above it
    if (!list.map((x) => x.toLowerCase()).includes(own)) return line;
    const merged = list.concat(extra.filter((a) => !list.includes(a)));
    return indent + 'server_name ' + merged.join(' ') + ';';
  });
}
...
async function writeBasicAuthFile(site) {
  ...
  if (!ba.username || !ba.password_enc) { if (!fs.existsSync(file)) throw Object.assign(new Error('basic auth is active but no password is stored — set one'), { status: 400 }); return; }   // keep CLP's file
...
module.exports = { SITES_DIR, SSL_DIR, BASIC_AUTH_DIR, BAK_DIR, ZONES_FILE, splitDomain, validDomain, materialize, render, retemplate, parsePhpValue, settingsBlock, phpSettings,
  writeTrafficZones, trafficBlock, hotlinkBlock, redirectBlock, rewriteBlock, aliasNames,
