...
const EXCLUDE_TOP = ['logs', 'tmp', '.pm2/logs', '.pm2/pids', '.pm2/*.sock', '.local/share/pnpm', '.local/state', '.local/lib/node_modules', '.local/share/Trash'];
...
async function databasesOf(site, accounts, homes) {
  ...
  for (const d of all) {
    ...
    const why = refs.has(d.name) ? 'referenced in .env' : mine.has(d.owner) ? 'owned by ' + d.owner : null;
...
async function archiveAndSend(site, p, opts, job) {
  ...
  job.archive.telegram = sent.map((s) => s.message_id);
...
async function removeEverything(site, p, opts, job) {
  ...
  for (const s of r.steps) { steps.push(s.name); say(job, '✓ ' + s.name + (s.detail ? ' — ' + s.detail : '')); }
