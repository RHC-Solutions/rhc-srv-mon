...
async function testAdmin(engine) {
  ...
  try {
    ...
    return { ok: true, engine, version: out.trim().split('\n')[0] || null, host: server.host, port: server.port, admin_user: server.admin_user, has_password: !!server.admin_password_enc };
  } catch (e) {
    return { ok: false, engine, error: e.message, detail: e.detail || null, host: server.host, port: server.port, admin_user: server.admin_user, has_password: !!server.admin_password_enc };
  }
}
...
async function create(site, opts) {
  ...
  return { ok: true, id: dbId, engine, name, user: username, password, permissions: perms, grant_hosts: engine === 'mariadb' ? MY_HOSTS : null };
}
...
module.exports = { myConnection, list, create, drop, exportDb, importDb, tables, maintain, adopt,
