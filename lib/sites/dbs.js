...
const { PG_SOCKET, mkdirMode, writeFileMode } = require('../util');
const store = require('./store');
const sqlite = require('./sqlite');
const { getent } = require('./users');

const IDENT = /^[A-Za-z0-9_]{1,63}$/;
const ENGINES = ['mariadb', 'postgres', 'sqlite'];
// The engines that are actually servers — the ones with a daemon, an admin credential and grants.
const SERVER_ENGINES = ['mariadb', 'postgres'];
...
const engineOf = (e) => { const s = String(e || '').toLowerCase(); if (!ENGINES.includes(s)) throw bad('unknown engine "' + s + '" (mariadb | postgres | sqlite)'); return s; };
...
function noAccounts() { throw bad('SQLite has no database accounts — access to the file is decided by unix permissions on it', 400); }
function serverFor(engine) {
  if (engineOf(engine) === 'sqlite') throw bad('SQLite has no server', 400);
...
async function list(site) {
  ...
  for (const engine of SERVER_ENGINES) {
    const t = await testAdmin(engine).catch((e) => ({ ok: false, engine, error: e.message }));
    out.servers.push(t);
  }
  // SQLite needs no daemon and no credential, so it is always reachable — say so in the same shape
  // the other two report, and point at the directory the files live in.
  out.servers.push({ ok: true, engine: 'sqlite', host: 'file', port: 0, admin_user: site.user,
    has_password: false, version: 'SQLite (' + sqlite.dirFor(site) + ')', serverless: true });
  ...
  for (const [engine, list_] of byEngine) {
    let s = new Map();
    if (engine === 'sqlite') { for (const d of list_) { const i = sqlite.info(site, d.name); if (i !== undefined) s.set(d.name, i); } }
    else { let server = null; try { server = serverFor(engine); } catch (_) {} if (server) s = await sizes(server, list_.map((d) => d.name)); }
...
async function create(site, opts) {
  ...
  // No daemon to start, no account to make: a SQLite database is finished the moment its file is.
  if (engine === 'sqlite') return sqlite.create(site, opts.name);
...
async function drop(site, id, opts) {
  ...
  if (d.engine === 'sqlite') return sqlite.drop(site, d, opts);
...
// SQLite exports land in the same place as the others, as portable SQL rather than a file copy —
// a copy taken while a writer holds the WAL would be a torn snapshot.
async function exportSqlite(site, d) {
  const dir = mkdirMode(path.join(backupDir(site), d.name), 0o770);
  const stamp = new Date().toISOString().replace(/[:T]/g, '-').slice(0, 19);
  const file = path.join(dir, d.name + '-' + stamp + '.sql.gz');
  const text = await sqlite.dump(site, d.name);
  writeFileMode(file, require('zlib').gzipSync(Buffer.from(text, 'utf8'), { level: 6 }), 0o660);
  const ent = getent(site.user);
  try { if (ent) fs.chownSync(file, ent.uid, ent.gid); } catch (_) {}
  let size = 0; try { size = fs.statSync(file).size; } catch (_) {}
  return { ok: true, file, size, engine: 'sqlite', name: d.name };
}
// Dump into the site's own backups/databases/, owned by the site user like CloudPanel does.
async function exportDb(site, id) {
  const d = store.databases.get(id);
  if (!d || d.site_id !== site.id) throw bad('unknown database for this site', 404);
  if (d.engine === 'sqlite') return exportSqlite(site, d);
  const server = serverFor(d.engine);
  const dir = mkdirMode(path.join(backupDir(site), d.name), 0o770);
  const stamp = new Date().toISOString().replace(/[:T]/g, '-').slice(0, 19);
  const file = path.join(dir, d.name + '-' + stamp + '.sql.gz');
...
function importDb(site, id, req, maxBytes) {
  const d = store.databases.get(id);
  if (!d || d.site_id !== site.id) throw bad('unknown database for this site', 404);
  if (d.engine === 'sqlite') throw bad('importing into SQLite is not wired up yet — replace the file at ' + sqlite.fileFor(site, d.name) + ' directly', 501);
  const server = serverFor(d.engine);
...
async function tables(site, id) {
  const d = store.databases.get(id);
  if (!d || d.site_id !== site.id) throw bad('unknown database for this site', 404);
  if (d.engine === 'sqlite') return { engine: 'sqlite', name: d.name, tables: sqlite.tables(site, d.name), actions: ['vacuum'], file: sqlite.fileFor(site, d.name) };
  const server = serverFor(d.engine);
...
async function maintain(site, id, action) {
  ...
  if (d.engine === 'sqlite') {
    if (a0 !== 'vacuum') throw bad('unknown action "' + a0 + '" (vacuum)');
    return Object.assign({ action: 'vacuum' }, sqlite.vacuum(site, d.name));
  }
  const server = serverFor(d.engine);
  const name = ident(d.name);
...
function adopt(site, engine, name) {
  if (engineOf(engine) === 'sqlite') throw bad('a SQLite database is a file — create it here rather than adopting one', 400);
  const server = serverFor(engine);
