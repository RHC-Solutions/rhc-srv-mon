...
const dbServers = {
  ...
  // SQLite has no server: each site gets its own row so database names stay unique per site.
  byEngineHost: (engine, host) => q('SELECT * FROM database_servers WHERE engine = ? AND host = ?').get(engine, String(host)) || null,
...
const databases = {
  ...
  updateUser: (id, patch) => { const cols = Object.keys(patch).filter((k) => ['username', 'password_enc', 'permissions'].includes(k)); if (cols.length) q(`UPDATE database_users SET ${cols.map((c) => c + ' = ?').join(', ')} WHERE id = ?`).run(...cols.map((c) => patch[c]), id); },
...
// Per-site nginx rules. All of these end up inside the vhost's {{settings}} block.
const rowList = (table) => (siteId) => q('SELECT * FROM ' + table + ' WHERE site_id = ? ORDER BY position, id').all(siteId);
const aliases = {
  list: (siteId) => q('SELECT * FROM site_aliases WHERE site_id = ? ORDER BY domain').all(siteId).map((r) => Object.assign(r, { redirect: !!r.redirect })),
  all: () => q('SELECT * FROM site_aliases').all(),
  insert: (a) => Number(q('INSERT INTO site_aliases (site_id, domain, redirect, created_at) VALUES (?, ?, ?, ?)').run(a.site_id, a.domain, bool(a.redirect), now()).lastInsertRowid),
  remove: (id, siteId) => q('DELETE FROM site_aliases WHERE id = ? AND site_id = ?').run(id, siteId).changes,
};
const redirects = {
  list: rowList('site_redirects'),
  insert: (r) => Number(q('INSERT INTO site_redirects (site_id, kind, source, target, code, position, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(r.site_id, r.kind || 'exact', r.source, r.target, Number(r.code) || 301, Number(r.position) || 0, now()).lastInsertRowid),
  remove: (id, siteId) => q('DELETE FROM site_redirects WHERE id = ? AND site_id = ?').run(id, siteId).changes,
};
const rewrites = {
  list: rowList('site_rewrites'),
  insert: (r) => Number(q('INSERT INTO site_rewrites (site_id, pattern, replacement, flag, position, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(r.site_id, r.pattern, r.replacement, r.flag || 'last', Number(r.position) || 0, now()).lastInsertRowid),
  remove: (id, siteId) => q('DELETE FROM site_rewrites WHERE id = ? AND site_id = ?').run(id, siteId).changes,
};
const hotlink = {
  get: (siteId) => { const r = q('SELECT * FROM site_hotlink WHERE site_id = ?').get(siteId); return r ? Object.assign(r, { enabled: !!r.enabled }) : null; },
  set: (siteId, p) => { const cur = hotlink.get(siteId) || { extensions: 'jpg,jpeg,png,gif,webp,svg,bmp,ico,mp4,webm,mp3,pdf', allowed: null, action: 403, enabled: false };
    const v = Object.assign({}, cur, p);
    q('INSERT INTO site_hotlink (site_id, enabled, extensions, allowed, action, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(site_id) DO UPDATE SET enabled = excluded.enabled, extensions = excluded.extensions, allowed = excluded.allowed, action = excluded.action, updated_at = excluded.updated_at')
      .run(siteId, bool(v.enabled), v.extensions, v.allowed || null, Number(v.action) || 403, now());
    return hotlink.get(siteId); },
};
const traffic = {
  get: (siteId) => { const r = q('SELECT * FROM site_traffic WHERE site_id = ?').get(siteId); return r ? Object.assign(r, { enabled: !!r.enabled }) : null; },
  all: () => q('SELECT * FROM site_traffic WHERE enabled = 1').all().map((r) => Object.assign(r, { enabled: true })),
  set: (siteId, p) => { const cur = traffic.get(siteId) || { enabled: false, req_per_sec: null, req_burst: 20, conn_limit: null, rate_kb: null };
    const v = Object.assign({}, cur, p);
    q('INSERT INTO site_traffic (site_id, enabled, req_per_sec, req_burst, conn_limit, rate_kb, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(site_id) DO UPDATE SET enabled = excluded.enabled, req_per_sec = excluded.req_per_sec, req_burst = excluded.req_burst, conn_limit = excluded.conn_limit, rate_kb = excluded.rate_kb, updated_at = excluded.updated_at')
      .run(siteId, bool(v.enabled), v.req_per_sec || null, Number(v.req_burst) || 20, v.conn_limit || null, v.rate_kb || null, now());
    return traffic.get(siteId); },
};
...
module.exports = { list, get, getById, byUser, publicView, password, insert, update, claim, remove, setPhp, setNodejs, setPython,
  certs, basicAuth, blockedIps, blockedBots, sshUsers, ftpUsers, cronJobs, dbServers, databases, templates, knownUnixUsers, usedPorts, cloudflare, aliases, redirects, rewrites, hotlink, traffic };
