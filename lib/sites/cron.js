...
function validateField(name, v) {
  ...
  if (!v) throw new Error('cron: ' + name + ' is required');
  if (FIELD_RE.test(v) || (NAMES[name] && v.split(',').every((p) => NAMES[name].test(p)))) return v;
  throw new Error('cron: invalid ' + name + ' "' + v + '"');
}
function validate(job) {
  ...
  if (!cmd) throw new Error('cron: command is required');
  if (/[\n\r]/.test(cmd)) throw new Error('cron: command must be a single line');
  if (cmd.length > 1000) throw new Error('cron: command too long');
  j.command = cmd;
  return j;
}
