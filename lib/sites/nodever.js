...
// A site recorded as LATEST does not name a version at all: it means "whatever the system node is
// right now". That is the difference between a record that is true today and one that was true the
// day it was written — the pinned majors this host inherited from CloudPanel all said 22 while every
// app had long since been running the system 26.
const LATEST = 'latest'
...
// The system interpreter's version, or null when there is none to read.
...
// The version a site should be running: resolved now for a site that tracks latest, otherwise the
// version it pinned. Callers render this; nothing stores it.
function effective(nodeVersion) {
  return isLatest(nodeVersion) ? systemVersion() : String(nodeVersion || '')
}
// The interpreter a site that tracks latest should execute.
function systemPath() {
  for (const bin of SYSTEM_CANDIDATES) { if (versionOf(bin)) return bin }
  return null
}
...
function actual(site) {
  const seen = new Map();
  let list = [];
  try { list = procs.listSnapshot(site).processes; } catch (_) { return []; }
  for (const p of list) {
    if (!p.pid) continue;
...
module.exports = { available, actual, versionOf, nvmVersions, systemVersion, systemPath, effective, isLatest, LATEST };
