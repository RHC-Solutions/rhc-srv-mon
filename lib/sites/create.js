...
async function createSite(opts, ctx) {
  ...
  try {
    ...
    // mkdir's mode is masked by the process umask — 0022 under pm2 — which quietly turns 0770 into
    // 0750 and leaves this site's SSH users able to read the docroot but never write to it. chmod
    // is not masked, so it is what actually sets the bits.
    try { fs.chmodSync(docroot, 0o770); fs.chownSync(docroot, ent.uid, ent.gid); } catch (_) {}
    say('created ' + docroot);
