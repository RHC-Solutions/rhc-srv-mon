...
async function createSite(opts, ctx) {
  ...
  if (type === 'nodejs') site.nodejs = { node_version: String(opts.node_version || (process.version.replace(/^v/, '').split('.')[0])), port: Number(opts.port) || freePort(3000), runtime: 'system' };
  if (type === 'python') site.python = { python_version: String(opts.python_version || '3'), port: Number(opts.port) || freePort(8000) };
