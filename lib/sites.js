...
function exeOf(pid) {
  ...
  let version = null;
  if (!stale) { try { version = execFileSync(path, ['-v'], { timeout: 4000, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim(); } catch { /* not a -v binary */ } }
  return { path, stale, version };
}
...
// harmless to the app (workers are fresh node processes) — measure the workers, report the daemon apart.
function isPm2Daemon(pid) {
  try { return /^PM2 v[\d.]+: God Daemon/.test(fs.readFileSync('/proc/' + pid + '/cmdline', 'utf8')); } catch { return false; }
}
function childrenOf(pid) {
  try { return fs.readFileSync('/proc/' + pid + '/task/' + pid + '/children', 'utf8').trim().split(/\s+/).filter(Boolean).map(Number); } catch { return []; }
}
...
async function checkSiteHealth(site, lmap) {
  ...
  const result = {
    ...
    nodeActual: null, nodeStale: false, nodeExe: null, pm2DaemonStale: false, phpActual: null,
  ...
  if (site.nodePort && result.portUp) {
    ...
    let e = l && exeOf(l.pid);
    if (e && isPm2Daemon(l.pid)) {
      result.pm2DaemonStale = e.stale;
      // Workers of any app under this daemon; the site's own app is among them. Stale if ANY is.
      const workers = childrenOf(l.pid).map(exeOf).filter(Boolean);
      if (workers.length) e = workers.find((w) => w.stale) || workers[0];
    }
    if (e) { result.nodeActual = e.version || null; result.nodeStale = e.stale; result.nodeExe = e.path; }
  }
