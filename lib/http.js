...
function dispatch(req, res, url) {
  ...
  for (const r of routes) {
    ...
    Promise.resolve().then(() => r.handler(ctx)).then((out) => {
      ...
      if (out === undefined) return json(res, 204, {});
      json(res, 200, out);
    }).catch((e) => {
...
module.exports = { clientIp, isLocalDirect, isHttps, json, authJson, readJsonBody, readJson, httpError, add, dispatch, setPermCheck };
