...
async function zones(tok) {
  ...
  return tok ? run() : cached('zones', run);   // an unsaved token must not poison the cache
}
...
module.exports = { configured, setToken, masked, verify, accountVerify, diagnose, accountOwned, zones, dnsRecords, zoneFor, overview, api };
