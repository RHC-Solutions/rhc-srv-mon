...
async function runBackup(trigger, actor) {
  ...
  saveBackups();
  events.emit('backups.run', { user: trigger === 'manual' ? (actor && actor.user) || 'system' : 'system', ip: actor && actor.ip, target: stamp,
    level: success ? 'info' : 'error', message: 'Backup (' + trigger + ') ' + (success ? 'completed: ' : 'FAILED: ') + result.items.length + ' items, ' + fmtBytes(bytes),
...
module.exports = { DEFAULT_BACKUP_SCOPE, loadBackups, saveBackups, runBackup, backupAvailable, listRemoteBackups, backupTick, getState, isRunning, clearLog, setConfig, exportSettings };
