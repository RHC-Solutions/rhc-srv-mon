...
recent = b''; pw_tries = 0; pw_pending_t = None; typed_since_prompt = False; t0 = time.time(); status = 0; closing_t0 = None
...
while True:
    ...
    # A login that becomes root has two password prompts: ssh's, then sudo's. Whether to answer is
    # decided per prompt, not once per session — latching on the first keystroke meant that the
    # moment someone typed at the ssh prompt the daemon went quiet for the rest of the session, and
    # the sudo prompt behind it was left for them to answer by hand, over and over.
    if pw_pending_t is not None:
        if typed_since_prompt:
            pw_pending_t = None                       # this one is the user's; stay out of it
        elif time.time() - pw_pending_t >= 0.35 and pw_tries < 2 and time.time() - t0 < 90:
            try: os.write(master, password.encode() + b'\\n')
            except OSError: pass
            pw_tries += 1; pw_pending_t = None
    if srv in r:
        try:
    ...
    if master in r:
        ...
        send(1, data)
        if password and pw_tries < 2 and time.time() - t0 < 90:
            recent = (recent + data)[-256:]
            tail = recent.split(b'\\n')[-1].rstrip()
            if b'assword' in tail.lower() and tail.endswith(b':'):
                # Matches ssh's "user@host's password:" and sudo's "[sudo] password for user:",
                # where the colon follows the name rather than the word.
                # Note the prompt; answering waits a moment below so a user who types their own
                # password is not raced by the daemon typing a second one.
                pw_pending_t = time.time(); typed_since_prompt = False; recent = b''
...
function ensureSshHelpers() {
  try {
    ...
    // sshJobEnv writes the password to a mode-600 file and deletes it when the job ends, but a job
    // killed mid-flight leaves one behind — two had been sitting here since a crash in September.
    // They hold the password in the clear, so anything older than the longest job timeout goes.
    const cutoff = Date.now() - 3600_000;
    for (const f of fs.readdirSync(SSH_HELPER_DIR)) {
      if (!f.startsWith('pw-')) continue;
      const full = path.join(SSH_HELPER_DIR, f);
      try { if (fs.statSync(full).mtimeMs < cutoff) { fs.unlinkSync(full); console.log('ssh: removed a stale password file left by an interrupted job'); } } catch (_) {}
    }
...
// ssh_config ships `SendEnv LANG LC_*`, so whatever locale this panel happens to run under is
// pushed onto every host it connects to. This one runs with LC_ALL=en_US.UTF-8, which pbx01 does
// not have generated, so each session opened with a wall of "cannot change locale" warnings from
// bash and from sudo. A remote has its own /etc/default/locale and is a better judge of it than we
// are, so nothing locale-related is handed to ssh and the remote keeps its own.
function envWithoutLocale(extra) {
  const env = Object.assign({}, process.env, extra || {});
  for (const k of Object.keys(env)) if (/^(LANG|LANGUAGE|LC_[A-Z_]+)$/.test(k)) delete env[k];
  return env;
}
...
function sshJobEnv(h) {
  const env = envWithoutLocale({ TERM: 'dumb' });
  let pwFile = null;
...
function sshOpenTerminal(req, socket, head, query) {
  ...
  const env = envWithoutLocale({ TERM: 'xterm-256color', RHC_PTY_COLS: String(cols), RHC_PTY_ROWS: String(rows) });
