...
async function sshKillRemoteSessions(h, pids, known) {
  ...
  // only ever signal a pid that is in the current listing (pids get reused); one that has already
  // gone away in the meantime is reported as gone instead of failing the whole batch
  ...
  // A plain kill of the sshd leader leaves logind holding the session open when a child outlives it
  // (nohup, a stray sleep), so the login keeps showing up. Terminate the logind session where there
  // is one — that takes the whole session scope with it — and fall back to signalling the process.
  const specs = signal.map((p) => (byPid.get(p).id ? byPid.get(p).id : '-') + ':' + p).join(' ');
  const cmd = 'for spec in ' + specs + '; do sid="${spec%%:*}"; pid="${spec##*:}";'
    + ' if [ "$sid" != "-" ] && command -v loginctl >/dev/null 2>&1; then loginctl terminate-session "$sid" >/dev/null 2>&1; fi;'
    + ' kill -HUP "$pid" 2>/dev/null; done; sleep 1;'
    + ' for spec in ' + specs + '; do pid="${spec##*:}"; kill -0 "$pid" 2>/dev/null && kill -TERM "$pid" 2>/dev/null; done; sleep 1;'
    + ' for spec in ' + specs + '; do sid="${spec%%:*}"; pid="${spec##*:}"; gone=1;'
    + ' kill -0 "$pid" 2>/dev/null && gone=0;'
    + ' if [ "$sid" != "-" ] && command -v loginctl >/dev/null 2>&1 && loginctl show-session "$sid" >/dev/null 2>&1; then gone=0; fi;'
    + ' if [ "$gone" = 1 ]; then echo "GONE $pid"; else echo "ALIVE $pid"; fi; done';
  const r = await sshRun(h, cmd, { timeoutMs: 60_000 });
  const results = signal.map((pid) => ({ pid, ok: new RegExp('GONE ' + pid + '\\b').test(r.stdout) }))
    .concat(stale.map((pid) => ({ pid, ok: true, note: 'already gone' })));
  return { results, ok: results.every((x) => x.ok), output: (r.stderr || '').trim().slice(0, 500) };
}
async function sshKillRemoteSession(h, pid) {
  ...
  const r = await sshKillRemoteSessions(h, [pid]);
  return { ok: r.results[0].ok, pid, output: r.output };
}
...
async function sshDisconnectOthers(h, opts) {
  ...
  const targets = cur.sessions.filter((x) => !x.fromPanel && x.pid > 1);
  const kept = cur.sessions.filter((x) => x.fromPanel);
  if (opts.dryRun) return { dryRun: true, targets: targets.map((t) => ({ pid: t.pid, user: t.user, from: t.from })), kept: kept.length };
  if (!targets.length) return { ok: true, results: [], kept: kept.length, none: true };
  const r = await sshKillRemoteSessions(h, targets.map((t) => t.pid), cur);
  return Object.assign(r, { kept: kept.length, targets: targets.map((t) => ({ pid: t.pid, user: t.user, from: t.from })) });
}
