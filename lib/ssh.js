...
async function sshKillRemoteSessions(h, pids, known) {
  ...
  // Hanging up sshd is not enough by itself — see resources/disconnect-sessions.sh — so the actual
  // escalation lives in that script, run on the target with one spec per session.
  ...
  const r = await sshRun(h, 'sh -s ' + specs, { stdinData: script, timeoutMs: 120_000 });
