...
// Cloudflare Turnstile (Settings → Turnstile), when configured: the form must carry a solved
// challenge before we even look at the password. Answers the request itself and returns false when
// the challenge is missing or rejected. Loopback skips it — see lib/turnstile.js.
async function turnstileGate(req, res, b) {
  if (!turnstile.enabled() || isLocalDirect(req)) return true;
  let r;
  try { r = await turnstile.verify(b && b.turnstile, clientIp(req)); }
  catch (e) { console.error('turnstile gate failed open:', e.message); return true; }
  if (r.ok) return true;
  if (!r.benign) authFail(req);
  authLog('turnstile_fail', req, { user: String((b && b.username) || '').slice(0, 40), codes: (r.codes || []).join(',') || 'none' });
  authJson(res, 403, { error: r.error, turnstile: 'retry' });
  return false;
}
...
function handleAuthRoute(req, res, url) {
  ...
  if (url === '/api/auth/state') {
    authJson(res, 200, { setupRequired: !authUsersExist(), locked: authLocked(req), issuer: AUTH_ISSUER,
      turnstile: turnstile.enabled() && !isLocalDirect(req) ? turnstile.siteKey() : null }); return true;
  }
  if (url === '/api/auth/me') {
    const s = authSessionOf(req);
  ...
  if (url === '/api/auth/setup' && req.method === 'POST') {
    ...
    readJsonBody(req, res, async (b) => {
      if (!await turnstileGate(req, res, b)) return;
