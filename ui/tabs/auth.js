...
async function authMfaReset(){
  ...
  try {
    ...
    document.getElementById('am-enrol').innerHTML = '<div style="display:flex;gap:14px;align-items:center;margin-bottom:10px"><div style="background:#fff;padding:6px;border-radius:8px;line-height:0">' + qr + '</div><div style="flex:1;min-width:0"><div class="hint" style="font-size:14px;color:var(--md-on-surface-med)">Scan with the new authenticator, or enter the secret:</div><div style="font-family:ui-monospace,Menlo,Consolas,monospace;font-size:14px;word-break:break-all;background:var(--md-surface-04dp);border-radius:6px;padding:6px 8px;margin:6px 0">' + esc(d.secret.replace(/(.{4})/g, '$1 ').trim()) + '</div>'
